...
// How much of the map the tallest building is allowed to occupy, and the least
// it may occupy before the extrusion stops reading as height at all.
...
function OSMViewer({ className = 'map-card', osmText = null, bbox = null } = {}) {
  const [buildings, setBuildings] = useState(null) // { features, total, returned }
  ...
  useEffect(() => {
    ...
    fetch(`${apiBaseUrl}/gis/buildings?${query}`)
      ...
      .catch((error) => {
        if (!active) return
        setFetchError(error instanceof Error ? error.message : 'Unable to load buildings.')
      })
      .finally(() => {
